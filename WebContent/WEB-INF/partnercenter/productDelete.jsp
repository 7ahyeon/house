						...
						<c:when test="${!empty productList }">
							<c:forEach var="vo" items="${productList }" varStatus="status">
								<tr>
									...
									<td>
										<button type="button" onclick="delete_go()" class="btn btn-sm btn-outline-danger font-weight-bold" style="margin:10px;">
											삭제
										</button>
									</td>
								</tr>
...
<script>
	...
	function sendData(tfm) {
	    ...
	    /*
		    $.ajax({
				  type:'POST',
				  url:'productUpdateListCategory.do',
				  ...
				  success: function(data) {
					  ...
					  $.each(alist, function(index, vo){
						  ...
						  htmlTag += "<tr>";
						  htmlTag += "<td>" + this.productNum + "</td>";
						  htmlTag += "<td>" + this.categoryName + "</td>";
						  htmlTag += "<td>" + this.categoryDetail + "</td>";
						  htmlTag += "<td>" + this.productName + "</td>";
						  htmlTag += "<td>" + this.stock + "</td>";
						  htmlTag += "<td>" + this.productPrice + "</td>";
						  htmlTag += "</tr>";
					  });
					  $('#listUp').html(htmlTag);
				  },
